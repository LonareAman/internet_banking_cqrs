...
public class WithdrawFundsController {
    ...
    public ResponseEntity<BaseResponse> withdrawFunds(@PathVariable(value = "id") String id,
        ...
        try{
            command.setId(id);
            commandGateway.send(command); //try "sendAndWait" when using in production

            return new ResponseEntity<>(new BaseResponse("Withdraw successfully completed"), HttpStatus.OK);

        }catch (Exception e){
            var safeErrorMessage = "Error while processing withdraw funds from the bank account for id - " + id;
